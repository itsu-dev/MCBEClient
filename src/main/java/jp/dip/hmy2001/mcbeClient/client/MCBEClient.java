...
import jp.dip.hmy2001.mcbeClient.client.ClientSession;
import jp.dip.hmy2001.mcbeClient.listener.PacketEventListener;
import jp.dip.hmy2001.mcbeClient.listener.SystemEventListener;
import jp.dip.hmy2001.mcbeClient.network.mcbe.GamePacket;
import jp.dip.hmy2001.mcbeClient.network.mcbe.json.JwtHeader;
import jp.dip.hmy2001.mcbeClient.network.mcbe.protocol.ProtocolInfo;
...
public class MCBEClient {
    ...
    private List<PacketEventListener> packetEventListeners;

    public MCBEClient(String serverAddress, int serverPort, int protocolId, String username, String clientUUID){
        this.username = username;
        this.clientUUID = clientUUID;
        this.protocolId = protocolId;
        this.packetEventListeners = new ArrayList<>();

        registerPacketEventstener(new SystemEventListener());
        generatePrivateKey();
    ...
    private void generatePrivateKey(){
        try{
            ...
            secureRandom = SecureRandom.getInstanceStrong();//getInstance("NativePRNG");
    ...
    public void registerPacketEventstener(PacketEventListener listener) {
        this.packetEventListeners.add(listener);
    }

    public void callPacketEventListener(GamePacket packet) {
        switch (packet.getPacketId()) {
            case ProtocolInfo.TEXT_PACKET: {
                packetEventListeners.forEach(listener -> listener.onTextPacket((TextPacket) packet));
                break;
            }

            case ProtocolInfo.SET_TIME_PACKET: {
                packetEventListeners.forEach(listener -> listener.onSetTimePacket((SetTimePacket) packet));
                break;
            }
        }
    }
